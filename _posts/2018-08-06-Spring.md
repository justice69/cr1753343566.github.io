...
- IoC 控制反转，依赖注入<br/>
将组件对象的控制权从代码本身转移到外部容器，就是将创建对象的工作交给IoC容器来进行，组件化的思想：分离关注点，<br/>使用接口，不再关注实现，依赖注入：将组建的构建和使用分开
- AOP 面向切面编程
AOP的目标：让我们专注于一件事
...
1. 创建带参的构造方法，参数为所依赖的数据访问层对象实例（编写带参构造方法后，Java虚拟机将不再提供无参的构造方法，<br/>为了保证使用的灵活性，建议自行添加一个无参构造）
...
1. 首先我们编写userServiceLogger类作为增强类，在类中写入增强方法 **before**和 **afterReturning**，在方法中实现<br/>日志的输出，并在xml文件中进行配置
